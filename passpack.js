...
/*
 * How to use MySocialCloud Passpack:
 *
 * At the end of this file there is reference to a global variable 'MySocialNamespace'.  Change 'MySocialNamespace' to whatever namespace you would like to use
 *
 */
(function() {
	var Namespace = {}

	Namespace.Passpack = new (function(){
	...
	Namespace.Passpack.extend({

		crypt: new Namespace.Passpack.static(),
		utils: new Namespace.Passpack.static(),
		_init: function () {}
	...
	**/
	...
	Namespace.Passpack.crypt.Base64 = {
	...
	**/
		...
		encode : function (input,noPlus) {
			...
			input = Namespace.Passpack.crypt.UTF8.encode(input);
		...
		decode : function (input,noPlus) {
			...
			output = Namespace.Passpack.crypt.UTF8.decode(output);
	...
	Namespace.Passpack.crypt.xxTEA = {
	...
	Namespace.Passpack.extend({
		...
		decode: function (algorithm, enctext, key, pars) {
			if (!pars) pars = {};
			var ppc = Namespace.Passpack.crypt,
	...
	// default algorithm
		...
		encode: function (algorithm, plaintext, key, pars) {
			if (!pars) pars = {};
			var ret = plaintext, ppc = Namespace.Passpack.crypt;
			switch (algorithm) {
				case 'UTF8':
				...
				case 'xxTEA':
					...
					break;
				case 'SHA256':	return ret = Namespace.Passpack.crypt.SHA256.calchex(plaintext); break;
	...
	*/
	...
	Namespace.Passpack.JSON = function () {
	...
	// it converts ISO date strings to Date objects:
	    ...
	    function baseFilter (k,v) {
			if (typeof(v) != 'string' || !Namespace.Passpack.JSON.isISODate(v)) return v;
			return Namespace.Passpack.JSON.ISO2Date(v);
		}
	...
	Namespace.Passpack.crypt.SHA256 = {
	...
	Namespace.Passpack.utils.extend({
	...
	// inspired by Keepass (http://keepass.info/)
		...
		passGenerator: function (chars,n) {
			var str = "";
			for (var j in chars) {
				var M = Namespace.Passpack.utils.charMatrix[j];
		...
		simplePassGenerator: function (n) {
			return Namespace.Passpack.utils.passGenerator({
					lcase: 1,
					ucase: 1,
					nums: 1
				},n?n:16);
		...
		genRandomKey: function (x, salt) {
			var l = x && x < 64 ? x : 64,
				ret = Namespace.Passpack.hashx((salt ? salt : '') + 
					Namespace.Passpack.utils.passGenerator({
						lcase: 1,
						ucase: 1, 
						nums: 1, 
						symb: 1, 
		...
		hashx: function (str,nohex,full) {
			var s = Namespace.Passpack.crypt.SHA256.calchex(str),
				ss = "";
			if (!full) for (var j=0;j<64;j+=4) ss += s.substring(j,j+2);
			else ss = s;
			return nohex ? Namespace.Passpack.utils.getStringFromHex(ss) : ss;
		},
	...
	Namespace.AES_ENCVAL = function(pass,key){

	   return  Namespace.Passpack.encode("AES",pass,key,{nbits:256  });
	 };

	Namespace.AES_DECVAL = function(encstring,key){
	 
	 	if(encstring == null){
	 		return '';
	 	}

	  return  Namespace.Passpack.decode("AES",encstring,key,{nbits:256  });
	 // return Aes.Ctr.decrypt(encstring, key, 256);;
	};

	Namespace.SHA256_Encode = function(pass){
	  return  Namespace.Passpack.encode("SHA256",pass); 
	};

	Namespace.Base64Decode = function(pass){
		 return  Namespace.Passpack.decode("Base64",pass);
	};

	Namespace.Base64Encode = function(pass){
		 return  Namespace.Passpack.encode("Base64",pass);
	};



	//This is what you should change to setup your namespace.  Change both occurences of MySocialNamespace.
	//
	//Note: This will not override an already existing variable in the global namespace.  
	//You will need to have a unique identifier, or put this object in your own namespace
